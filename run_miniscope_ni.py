...
camera_process = subprocess.Popen(['python', paths.local_miniscope])
...
frame_times = frame_list[np.argwhere(np.diff(np.round(frame_list[:, 2])) > 0).flatten()+1, 0]
...
if np.round(frame_list[start_frame, 2]) > 0:
